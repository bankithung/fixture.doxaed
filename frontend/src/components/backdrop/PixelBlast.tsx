...
/** PixelBlast (React Bits, inspired by zavalit/bayer-dithering-webgl-demo),
 * re-cut for the app backdrop: liquid/noise composer passes stripped (no
 * postprocessing dep), ripple clicks listen on WINDOW (the canvas is
 * pointer-events-none behind the UI), rAF pauses while the tab is hidden,
 * and the color uniform can retarget on theme flips without a re-init.
 * Loaded lazily — three.js stays out of the entry bundle. */
...
export default function PixelBlast({
  ...
  // Init once; prop changes below only retune uniforms.
  useEffect(() => {
    ...
    try {
      ...
      return; // no WebGL — backdrop simply stays empty
    }
    ...
    // The canvas is pointer-events-none behind the app, so ripples ride a
    // WINDOW listener and map viewport coords onto the framebuffer.
    const onPointerDown = (e: PointerEvent): void => {
      if (!uniforms.uEnableRipples.value) return;
    ...
    let raf = 0;
    const animate = (): void => {
      // Idle while the tab is hidden — no GPU work in the background.
      if (!document.hidden) {
    ...
    return () => {
      ...
      ro?.disconnect();
      cancelAnimationFrame(threeRef.current?.raf ?? raf);
    ...
    // Mount-once by design; the retune effect below handles prop changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Retune uniforms in place (theme flips retarget uColor with no re-init).
  useEffect(() => {
    const state = threeRef.current;
    if (!state) return;
