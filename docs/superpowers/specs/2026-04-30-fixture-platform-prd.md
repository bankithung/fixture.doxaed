...
> **Status:** Draft v7 — all logic-flow gaps folded in; v4 (2026-06-12) adds the `abandoned → scheduled` replay transition (§5.5, decision 71); v5 (2026-06-12) adds the `called_at` operational annotation note (§5.5, decision 72); v6 (2026-08-03) promotes court to a first-class entity alongside the denormalised `Match.venue` string (decision 84); v7 (2026-08-14) drops Members & roles from the setup funnel, leaving a five-step flow (decision 85)
> **Date:** 2026-04-30
...
| 78 | The set-sport console is an INSTRUMENT PANEL (2026-07-26): one telemetry strip (status · period of best-of · match score · the stage's winning points · stopwatch · sync), two score cards with the stage rule between them, the FULL period history (played, in play, and still-to-play rows, marked "Not needed" once a side clinches), a state rail and a sticky action bar; the board fills 90% of the shell width from `lg` up, the pads stay side by side on a phone, and Q/P score home/away on a keyboard | Owner reference design. The scorer needed the stage target and how many periods remain visible without opening anything; the old board hid both in a caption and buried corrections in a `<details>` |
| 79 | Table tennis tracks NO service (2026-07-26): the serve indicator and first-server toggle are gone from the TT console (the umpire calls service); `serve.ts` and sepak takraw's three-serves-a-turn rotation are untouched | Owner call — a service indicator the umpire must keep in sync with their own call is a second source of truth, not an aid |
| 80 | Every invited member gets a self-scoped **My tasks** page (2026-07-26): their assigned matches only, with the Matches board's filters/grouping/rows, table→cards on a phone. "Assigned" = the scoring seat (`Match.scorer`) OR any `MatchOfficial` slot; the nav item is deliberately UNGATED because the page can only ever show the viewer's own work | Officials had no view of their own day at all — the board's "My matches" toggle checked only the scorer seat, and a referee had to find their 2 matches among 102 |
| 81 | The console is the ONLY scoring entry point from the board (2026-07-26): "Enter result" is gone from the row menu and the control-room tile; Open console is the promoted action and the first menu item. Finished matches are tinted `success-muted` down the whole row | Owner: two doors to the same job, and no way to see at a glance what was already settled |
| 82 | The public album is a **two-stage universe** (2026-07-27), replacing the plain masonry as the default view: stage one is a school orbit — one planet per institution, sized by photo count, turning on tilted rings around a sun that is the whole album; picking a planet flies the camera into stage two, a drag-rotatable **photo sphere** (React Bits' DomeGallery re-cut token-native: CSS 3D only, no `@use-gesture`, no vertical clamp, auto-spinning by default). The masonry survives as an explicit **Grid** view — the exhaustive, low-power, keyboard-complete route — and the choice sticks per visitor | Owner asked for the DomeGallery look with full rotation, click-to-open, default motion, and "a planet per school you can zoom into". The grid stays because a 3D ball is a browsing surface, not an index: it repeats photos to fill itself and it is one tab stop, not 130 |
| 83 | Below `lg` the orbit becomes a **planet rail** (2026-07-27): the sphere takes the screen and the schools line up on a scrollable strip of the same discs. On a desktop, a planet parks itself on hover/focus, and one pause switch stops both stages | Twelve planets inside 390px collapsed into a pile of overlapping labels, and even on a desktop, aiming at a moving target is the one thing that would have made the stage annoying |
| 84 | **Court is a first-class entity** (2026-08-03): `fixtures.Court` (org + venue FK, name, 1-based index, soft delete, unique name per venue) and `Match.court` (nullable FK, `SET_NULL`). `Match.venue` SURVIVES unchanged as the denormalised display string — the four writers of it (`repair.reschedule_match`, `repair.swap_slots`, the `PATCH /api/matches/{id}/schedule/` endpoint, `scheduler.apply_schedule`) now write both and keep `venue == court.name`; `reflow_from_actual` still moves only the clock. Courts are derived identity: materialised on demand by `services/courts.resolve_court` and by the `backfill_courts` command, never invented where the base `Venue` row is missing (FK stays null, string stays authoritative) | A court had no row, so nothing could hang a stream URL, an overlay key or a per-court crew assignment off it — but ~40 readers key off the `venue` string, so promoting it had to be additive, not a migration of the read path |
| 85 | **Members & roles leaves the setup funnel** (2026-08-14): the flow is five steps (Setup, Institution registration, Team registration, Fixtures, Ready). Member/role administration becomes an always-open surface pinned beside Settings — below the stepper in the setup sidebar, last in the nav rail. Team registration now gates FIXTURES directly (`no_teams_registered` moved). The `members` stage value stays in the enum (stage_meta + audit history reference it) but nothing can transition into it; `_RETIRED_RANK` gives it a fractional rank so a tournament parked on it still advances | Owner: roles are handed out throughout setup and again on match day, so numbering it made the funnel misreport progress — a tournament two steps in read "2/6" against a step it would never tick off |
| 86 | **The opening and closing ceremonies bound their day's play times** (2026-08-15): a `ceremony_block` labelled `opening` blocks its date from the start of the day until the ceremony ENDS; one labelled `closing` blocks from its START to the end of the day (`scheduler.ceremony_window`, honoured by both `build_slots` and `validate_assignments`; the capacity estimate in `readiness` counts the same stretch). An unlabelled ceremony block — the advanced builder's own record — still cuts exactly its own window. The wizard seeds a new opening at `daily_start` and a new closing at `daily_end`, and the ceremonies then WRITE the play times: `withCeremonyBounds` moves `daily_start` to the opening's end and `daily_end` to the closing's start, on every ceremony edit and when the stored form is seeded (so a tournament set up before this re-derives on open). The fields stay editable; widening one back only affects the other days, since the engine keeps bounding the ceremony day. A ceremony that would invert the window is not applied, and says so | Owner: "we are setting the opening and closing ceremony so why don't we update the timings of the first and last match automatically". The engine only carved out the ceremony hour, so it would happily schedule matches before the opening ceremony or after the closing one, and the Play times step read "first match 09:00" on a morning the ceremony owned |
| 87 | **Every sub-step of the When & where wizard saves** (2026-08-15): the per-step Save writes the same three channels as the final one (venues API, constraint records, `draw_config["*"].calendar`) and stays on the step, re-seeding from the refetched data first. The hub pins the wizard open on that step (`onSavedInPlace`), because a save can satisfy the very gate that opened it | Owner: work typed on Dates/Venues/Play times was lost on leaving, and changing one venue meant walking to Check & save; the first cut then dropped the user on Clashes & sessions mid-edit when the gate fell away |
| 88 | **The previewed draw is PINNED** (2026-08-20), amending D6's "preview persists nothing": a preview writes the winning per-competition SEEDS plus the overrides they ran under to `draw_config["preview_pin"][<scope>]`, and every later preview replays them, so the fixture is the same one on any device and for any manager. It is redrawn on exactly three triggers: **Try another draw** (which re-pins the winner), the draw inputs moving under it (teams, format or pairing rules — detected by a fingerprint over each scope's `inputs_hash`, redrawn AUTOMATICALLY and reported in the body's `pin` object), and a pinned draw that can no longer be given times. The CALENDAR is deliberately outside the fingerprint, so lengthening a day re-times the fixture without re-pairing it. A pin is a seed, never a fixture: no `Match` row, no `event_id`, no audit event, and `preview_pin` is inert to `effective_draw_config` (which reads only the `*`/`sport:`/leaf layers) so it can never perturb the hash that invalidates it | Owner: "it should be generated once and saved and not change until I press try another draw". A competition seeded at random minted a fresh seed on every visit, so the organizer was shown a different fixture each time; and a re-draw they liked lived only in the open tab, replaced by the configured draw the moment they navigated away |
...
*End of PRD draft v7. All identified logic-flow gaps folded in.*
