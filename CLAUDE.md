...
- **State machines + advancement.** `apps/matches/services/state.py` (`ALLOWED_TRANSITIONS` + guarded/audited `transition_match`) and the Tournament status enum. On match completion/walkover, `transaction.on_commit` fires `apps/fixtures/services/advance.py::advance_from_match`, which resolves **typed match-dependency pointers** (`Match.home_source`/`away_source` JSONB: `winner_of`/`loser_of`/`group_position`/`team`/`tbd`) to fill the next round. **Advancement is re-entrant, not a one-shot at completion** (2026-08-26): results come off the courts out of order, so a semi-final gets scored while its own sides are still placeholders — the score is positional and stays right, but `winner_id`/`loser_id` are None at that instant and nothing reaches the round below. So the trigger is "this match's teams changed", not "this match finished": `_cascade` re-advances from any dependent a pass fills that is ALREADY final, and an unknown `loser_id` is left to wait rather than stamped NULL into the 3rd-place slot. Same-school pairings are never a reason to withhold a fill — separation is a DRAW rule (`_separate_bracket_by_key`), and a resolved pointer plays whoever won.
...
- **A competition is ONE page, with NO view switcher** (owner 2026-08-21): its **group stage as a sheet** (`GroupStageSheets`, one per group, `showCompetition={false}` and a Day column when it spans days), then its **knockout as the bracket BELOW**. It carries **no Up next band and no standings**: the sheet flags the competition's own next match in its Status column (one flag, not one per group), and standings live on their own tab (owner 2026-08-21). `LiveBand` and `FollowedBand` are the only bands left anywhere in the match centre. Knockout rows never join the fixture list — every side there reads "TBD" until the groups finish, which is exactly what the tree is for. A **knockout-only** competition (every table-tennis category) is the bracket alone: no table, nothing to switch to.
...
## Results are DERIVED: the medal tally (2026-08-25)

`features/results/PublicResultsPage.tsx` (`/t/:slug/:id/results`, fourth public tab) is the sheet ANPSA retype into Word after every meet — schools down, one column per event, the **placing** in the cell, medal counts on the row — plus the two things paper cannot do. Spec: `docs/superpowers/specs/2026-08-25-results-medal-tally-design.md`.

- **Nothing records who won.** `matches/services/placings.py` reads it off the fixture: a competition's last `(stage_no, round_no)` step holds the final and, beside it, the third-place playoff — told apart by both sides being `loser_of` (the same test `scheduler._is_third_place` uses), NEVER by the `· 3rd Place` group label, which is a retypeable display string. No playoff and `bronze: "shared"` gives both losing semi-finalists a full bronze.
- **A competition that HAS a bracket is decided by it, full stop.** The group-table path runs only when there is no knockout at all, and only once every match is played: `compute_standings` deliberately seeds a group at zero before a ball is kicked, so reading it early hands out gold in draw order. Two groups and no knockout refuse to rank at all.
- **The ladder lives OUTSIDE `rules`** (`Tournament.awards` JSONB, `tournaments/services/awards.py`). `rules` freezes at `registration_open` (invariant 7) and never thaws; a host settles points on the morning of the meet, and a ladder decides a trophy rather than a result. It is a LIST of `{place, points, label}`, so scoring the top six is data; `by_competition` weights one sport or one category by the same prefix matcher courts and `phased_finish` use.
- **A category group is an authored list of prefixes** — an empty list is every competition, which is how "Overall" is expressed without a special case. `suggest_groups` reads one group per (SPORT, age band, gender) off the host's own tree: table tennis U-14 boys and sepak takraw U-14 boys are different sports and therefore different champions (owner 2026-08-25), and merging them awards one trophy for two games. A host who wants them combined ticks both into one group. Nothing in code knows what a sport, an age band or a gender is.
- **It is ONE section** (owner 2026-08-25): heading, totals, view switcher, filters and the sheet live in the same panel. Read across four floating cards, a medal tally is four things rather than one. Gold/silver/bronze columns wear their own medal tint so the eye finds them without reading the header.
- **A student carries the FULL points of their team's placing** (owner 2026-08-25): both halves of a winning doubles pair show 5, and the school counts the gold once. The view answers "what was this child part of", and says so on screen — two totals that do not add up otherwise read as a bug.
- **One payload, three views** (Tally / Champions / Students), so they cannot disagree about who won. The grid reuses `entriesMatrix`'s column model via `CompetitionLike`/`WithCode<T>` — one competition wears the same short code on both public tabs.
- **Every number is scoped to the columns on screen** — row totals, column footer, grand total and the CSV alike. This is the bug the entries matrix shipped with (its footer mixed a server-wide count with visible-row totals); do not reintroduce it here.
- `--medal-1/2/3` are their own token trio (gold/silver/bronze are fixed by the world, not the brand), validated in both themes. Every medal chip carries its place NUMBER as well as its colour: silver reads neutral by definition.
