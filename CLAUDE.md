...
- **A competition is ONE page, with NO view switcher** (owner 2026-08-21): its group tables, then its **group stage as a sheet** (`GroupStageSheets`, one per group, `showCompetition={false}` and a Day column when it spans days), then its **knockout as the bracket BELOW**. Knockout rows never join the fixture list — every side there reads "TBD" until the groups finish, which is exactly what the tree is for. A **knockout-only** competition (every table-tennis category) is the bracket alone: no table, nothing to switch to.
